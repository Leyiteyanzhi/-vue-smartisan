            ...
            <div class="cart-box">
                ...
                <div class="cart-inner">
                    <div class="empty-label" v-if="count<=0">
                        <h3>您的购物车中还没有商品</h3>
                        <router-link :to="{name: 'Shop'}" class="link">现在选购</router-link>
                    </div>
                    <div v-else>
                                ...
                                <div class="cart-top-items" v-for="item,index in carPanelData">
                                    <div class="cart-items">
                                        <div class="items-choose">
                                            <span class="blue-checkbox-new" :class="{'checkbox-on':item.checked}" @click="checkedHandle(item.sku_id)"><a></a></span>
                                        </div>
                                        ...
                                        <div class="item-cols-num">
                                            <div class="select js-select-quantity">
                                                <span class="down" :class="{'down-disabled':count<=1}" @click="subCarPanelDataHandle(item.sku_id)">-</span>
                                                <span class="num">{{item.count}}</span>
                                                <span class="up" :class="{'up-disabled':count>=item.limit_num}" @click="plusCarPanelDataHandle(item.sku_id)">+</span>
                ...
                <div class="cart-bottom-bg fix-bottom" v-if="count>0">
                    ...
                    <div class="shipping"></div>
                        ...
                        <span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn':checkedCount <= 0}"><router-link :to="{name: 'CheckOut'}">现在结算</router-link></span>
                    </div>
                </div>
            </div>
        </div>
